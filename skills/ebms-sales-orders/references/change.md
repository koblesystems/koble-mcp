...
1. **Read the order.** If `STATUS` is `'O'` it has been processed: its lines are locked and
   edits return 403. Tell the user. Unprocessing (`{"PROCESS": "Unprocess"}`) reverses a posted
   invoice — do it only if they explicitly ask, after confirming. See `fulfil.md`.
...
5. **Send it, read back, and verify each change.** Before retrying anything that didn't land,
   read the order again: re-sending a modify or a removal is harmless, but re-sending an add
   creates a second copy of the line.
...
- **Changing the order's customer** can't be done by PATCHing `ID` (EBMS refuses). Use the
  `ChangeCustomer` command in `commands.md`.
...
- On a timeout, read the order back before sending anything again. Modifies and removals are
  safe to resend; adds are not.
